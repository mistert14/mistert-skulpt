...
PIECES = None 
#forward reference; defined after piece classes 
...
class Board:
    ...
    def checkmate(self, color):
        ...
        if not king.threat(self):
            return 0
        # If the king is under threat but has legal moves then it is not checkmated.
        if king.possible(self):
            return 0
        # Finally, if the king is under threat and has no legal moves, then we must go through every other piece owned by that color and see if it has any legal 
        # moves (i.e., moves which take the king out of check); if none do, then it is checkmated.
    ...
    def asArray(self):
        ...
        for i in range(SIZE):
            ...
            for j in range(SIZE):
                if self.array[i][j]:
                    line.append(repr(self.array[i][j]))
                else:
                    line.append('.')
            array.append(line)
        return array
...
class Move:
    ...
    def longAlgebraic(self):
        ...
        if self.isCheck:
            ...
            suffix = '#'
        return self.piece.symbol[WHITE] + repr(self.start) + \
               takes + repr(self.end) + suffix 
class Game:
    """Encapsulates a game."""
    ...
    def save(self, file):
        ...
        for piece in self.board.pieces():
            rep = "%s.%s.%d.%d" % (repr(piece), repr(piece.loc), \
                                  piece.moves, piece.idle)
            pieces.append(rep)
        file.write(string.join(pieces, ' ') + "\n")
        moves = []
        for move in self.moves:
            rep = repr(move[0]) + repr(move[1])
            moves.append(rep)
    ...
    def disambiguate(self, command):
        ...
        if command == 'O-O':
           ...
           command[2] in 'abcdefgh' and command[3] in '12345678':
            ...
            try:
                # Long or standard algebraic notation. Check for pawn promotion.
                promote = None
                ...
                if command:
                    if command[-1] == 'x':
                        ...
                        command = command[:-1]
                # If first character is an uppercase letter then it represents the kind of piece, otherwise it is a pawn.
                if command:
                    print command
                    if command[0] in 'ABCDEFGHIJKLMNOPQRSTUVWXYZ':
                ...
                # Now get a list of pieces which could move to the end position.
